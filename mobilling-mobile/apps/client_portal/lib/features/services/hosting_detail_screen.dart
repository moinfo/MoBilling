...
class HostingDetailScreen extends ConsumerWidget {
  const HostingDetailScreen({super.key, required this.accountId});

  final String accountId;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final detail = ref.watch(hostingDetailProvider(accountId));
    ...
    return Scaffold(
      ...
      body: detail.when(
        ...
        error: (error, _) => StateMessage(
          ...
          actionLabel: 'Retry',
          onAction: () => ref.invalidate(hostingDetailProvider(accountId)),
        ),
...
class _BodyState extends ConsumerState<_Body> {
  ...
  Future<void> _refreshUsage() => _run(() async {
        ...
        ref.invalidate(hostingDetailProvider(a.id));
        ref.invalidate(hostingProvider);
      });
